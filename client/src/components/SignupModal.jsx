...
const SignupModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-[#1C1F20] max-h-[90vh] overflow-y-auto p-6 rounded-xl w-full max-w-md text-white border border-[#EF5F07]/30 shadow-xl relative">
        <button
          onClick={onClose}
        ...
        <div className="flex justify-center mb-4">
          <label className="cursor-pointer">
            <div className="w-24 h-24 rounded-full bg-[#2A2D2F] border border-gray-600 flex items-center justify-center overflow-hidden hover:ring-2 ring-[#EF5F07] transition">
              {imagePreview ? (
                <img src={imagePreview} alt="Profile" className="w-full h-full object-cover" />
              ) : (
                <span className="text-gray-500 text-sm text-center px-2">Upload</span>
              )}
            </div>
            <input
              type="file"
        ...
        <form className="space-y-4">
          <div className="flex gap-4">
            <input
              ...
              className="w-1/2 px-4 py-2 rounded bg-[#2A2D2F] border border-gray-700 focus:ring-2 focus:ring-[#EF5F07]"
            />
            <input
              type="text"
          ...
          <input
            ...
            className="w-full px-4 py-2 rounded bg-[#2A2D2F] border border-gray-700"
          />
          <input
            type="text"
